// FLOWCHARTS DATA: every pipeline and workflow of the partner program, as data for
// tools/flowcharts/render.js. This is the TEMPLATE: the first build's 14 pages, with the client's
// values taken out. The finished PDF goes to the client at handover (process/9-handover.md).
//
// USE IT
//   1. Copy this file to clients/<CLIENT_SLUG>/flowcharts-data.js, next to that client's config.json.
//   2. Set the CLIENT VALUES block below. For a standard build (monthly %), nothing else changes.
//      For "one fee per closed deal", also set VARIANT = 'fee': see "Commission variants" below.
//   3. Render, from the repo root:
//        node tools/flowcharts/render.js clients/<CLIENT_SLUG>/flowcharts-data.js --pdf
//      It writes flowcharts.html and flowcharts.pdf next to the data file, then prints a layout
//      check. The bar is its line "No problems", and no "warning:" line about placeholders.
//
// READ FROM config.json (next to this file), so there is nothing to type by hand:
//   - every GHL id on the pages (the "ids" block that survey.js --ids writes, and "locationId").
//     A missing id just leaves that box without an id line;
//   - the segment tags on the last page (the "segment/..." tags in structure.tags).
//
// ADAPTING IT
...
//   * One fee per closed deal. Not yet proven in a build. Set VARIANT = 'fee' and RATE = the fee as
//     text with its currency sign ('$250'). The pages then change by themselves; every change is
//     marked FEE in the code below, so you can read exactly what prints:
//       page 'overview': summary; boxes 'wf-signup', 'pending', 'approved', 'c3' and 'paid' reworded;
//         the node 'stop' and the edges c3 -> pending and c3 -> stop are gone;
//       page 'pl-recruiting': the 'offer' stage note;
//       page 'pl-referred': summary; the notes of Commission Pending, Approved and Paid; no loop;
//         no "Stops the monthly commission cycle" flag on Lost / No Sale;
//       page 'wf-signup': summary; "Set up as partner" says Reward Model = Per Sale, with no
//         Revenue Share Percent;
//       pages 'wf-comm1' and 'wf-comm2': summaries and task texts name the fee;
//       page 'wf-comm3': the renamed workflow 'Referral Engine - Commission 3: Paid' (title and id
//         lookup), one step, then End: no Wait, no "Still a paying client?";
//       page 'affiliate', box 'payouts', and page 'glance', the tags note: no "%", no monthly cycle.
//     If the client renamed stages (spec/variants.md), change those stage names by hand too.
//     The renderer warns when a fee is printed with a "%" after it ("$250%").
//   * Pay per lead. Not yet proven in a build. Follow spec/variants.md "## Pay per lead": workflows
//     7 and 8 are not built, and the fee is tracked on the deal's Commission Status field only, so
//     the deal never leaves the sales stages. Change:
//       pages 'wf-comm2' and 'wf-comm3': delete both;
//       page 'wf-comm1': the counting stage in the trigger box (spec/variants.md says which stage);
//         step 1: no stage move, no Sale Date, Commission Status = Pending, Reward Amount = the fee;
//         the task: staff set Commission Status to Approved, pay, then set it to Paid, by hand;
//         the title too if the workflow is renamed '... Commission 1: Lead'; reword the summary;
//       page 'pl-referred': delete `loops`; move Commission 1's `triggers` from Sold / Enrolled to the
//         counting stage; mark Commission Pending, Commission Approved and Commission Paid as not
//         used (or delete them if the config replaces the pipeline); reword the summary;
//       page 'overview': delete the nodes 'pending', 'approved', 'c2', 'paid', 'c3', 'stop' and
//         every edge that touches them; point the deal's manual edge at the counting stage, not
//         Sold / Enrolled; reword the summary and the boxes 'sold' and 'c1' (the deal stays put);
//       page 'affiliate', box 'campaign': the per-lead amount is on, so name the amount, and mark
//         it not yet proven;
//       page 'glance': reword the commission-tags note (no monthly cycle);
//       every `${RATE}%` text: say the fee instead (search this file for RATE). RATE holds the fee
//         as text with its currency sign ('$250'), as for the fee variant.
//
...
// ================================== CLIENT VALUES ==================================
const CLIENT = '<CLIENT_NAME>'; //          the business, e.g. 'Example SEO Agency'
const PROGRAM = '<PROGRAM_NAME>'; //        '<CLIENT_NAME> Partner Program', e.g. 'Example SEO Partner Program'
const VARIANT = 'monthly'; //               'monthly' (monthly %, the default) or 'fee' (one fee per closed deal)
const RATE = '<RATE_PERCENT>'; //           monthly: the percentage as a plain number, no "%" ('15').
//                                          fee: the fee as text with its currency sign ('$250')
const PERSON = '<PLACEHOLDER_USER>'; //     who gets every task and alert until the client has a GHL user
const PAYOUT_TERMS = '<PAYOUT_TERMS>'; //   the campaign's payout terms, e.g. 'Net-15'
const BUILD_DATE = '<BUILD_DATE>'; //       e.g. 'Sep 21–22 2026'
const TEST_DATE = '<TEST_DATE>'; //         the day the tests passed, e.g. 'Sep 22'
...
const FEE = VARIANT === 'fee'; //           FEE marks every text that differs for the fee variant
const COMM3 = FEE ? 'Referral Engine - Commission 3: Paid' : 'Referral Engine - Commission 3: Paid, monthly repeat';
...
const pages = [
  // ================================================================= 1. OVERVIEW
  {
    ...
    ghlId: LOCATION_ID,
    summary: `Partners join ${USES_INSTANTLY ? '(from cold email or the sign-up form)' : 'through the sign-up form'}, refer clients through their personal link, ${FEE ? `and earn ${RATE} for each referred deal that closes.` : `and earn ${RATE}% every month the client pays.`}`,
    legend: true,
    ...
    nodes: [
      ...
      { id: 'wf-signup', at: [1, 1], type: 'action', kicker: 'Workflow', title: 'Partner Sign-Up', desc: FEE ? 'Fee terms, personal link, deal, onboarding task.' : 'Rate, personal link, deal, onboarding task.', ref: 'wf-signup' },
      ...
      { id: 'pending', at: [3, 2], type: 'stage', kicker: 'Stage', title: 'Commission Pending', desc: FEE ? 'Staff confirm the deal closed and the fee.' : 'Staff check the payment, fill in the amount.' },
      { id: 'approved', at: [3, 3], type: 'stage', kicker: 'Stage', title: 'Commission Approved', desc: FEE ? 'The fee is approved.' : 'This month approved.' },
      { id: 'c2', at: [3, 4], type: 'action', kicker: 'Workflow', title: 'Commission 2: Approved', desc: 'Records the approval, payout task.', ref: 'wf-comm2' },
      ...IF(!FEE, { id: 'stop', at: [4, 1], type: 'end', title: 'Cycle stops', desc: 'Deal moved to Lost / No Sale.' }),
      { id: 'c3', at: [4, 2], type: 'action', kicker: 'Workflow', title: 'Commission 3: Paid', desc: FEE ? 'Records the payment. Nothing repeats.' : 'Records the payment, waits 30 days.', ref: 'wf-comm3' },
      { id: 'paid', at: [4, 3], type: 'stage', kicker: 'Stage', title: 'Commission Paid', desc: FEE ? 'Partner paid the fee.' : 'Partner paid for this month.' },
    ],
    edges: [
      ...IF(USES_INSTANTLY,
  ...
  // ================================================================= 2-4. PIPELINES
  {
    ...
    stages: [
      ...
      { id: 'offer', title: 'Offer Made', note: FEE ? `Program offered: ${RATE} for each referred deal that closes.` : `Program offered: ${RATE}% recurring on referred clients.` },
    ],
    entries: [
      ...IF(USES_INSTANTLY,
  ...
  },
  {
    ...
    summary: FEE ? `Clients that partners refer. The last four stages run the ${RATE} partner fee, once per closed deal.` : `Clients that partners refer. The last four stages run the ${RATE}% commission, once a month for as long as the client pays.`,
    perRow: 5,
    colGap: 30,
    stages: [
      ...
      { id: 'pending', title: 'Commission Pending', note: FEE ? 'Staff confirm the deal closed and was paid.' : "Staff confirm this month's payment.", setBy: FEE ? 'Commission 1' : 'Commission 1, then Commission 3 monthly', arrowIn: 'auto' },
      { id: 'approved', title: 'Commission Approved', note: FEE ? `Fee approved: Reward Amount ${RATE}.` : `Client paid; ${RATE}% filled in.`, triggers: 'Commission 2 ({p:wf-comm2})' },
      { id: 'paid', title: 'Commission Paid', note: FEE ? 'Partner paid the fee. Nothing repeats.' : 'Partner paid for this month.', triggers: 'Commission 3 ({p:wf-comm3})' },
    ],
    loops: FEE ? [] : [
    ...
    side: {
      ...
      stages: [
        ...
        FEE ? { id: 'lost', title: 'Lost / No Sale', note: 'Did not buy.', col: 2 } : { id: 'lost', title: 'Lost / No Sale', note: 'Did not buy, or cancelled.', flag: 'Stops the monthly commission cycle.', col: 2 },
  ...
  },
  {
    ...
    summary: `A new partner signs up. GHL sets them up ${FEE ? `for the ${RATE} fee per closed deal` : `at ${RATE}%`}, gives them their personal referral link and adds them to the Affiliate Manager.`,
  ...
  },
  {
    ...
    summary: FEE ? "The referred deal closed. Opens the partner's one-time fee and locks who gets the credit." : "The referred client signed. Opens this month's commission and locks who gets the credit.",
    ...
    steps: [
      ...
      { type: 'action', kicker: 'Update opportunity', title: 'Record sale, open commission', desc: `Moves the deal to **Commission Pending**; Commission Status = Pending; Sale Date = today${FEE ? `; Reward Amount = ${RATE}` : ''}.` },
      ...
      { type: 'task', kicker: 'Add task', title: '#1 Approve commission task', desc: FEE ? `Confirm the deal closed and the client paid; check Reward Amount is ${RATE}; drag the deal to Commission Approved. ${X}.` : `Confirm the client paid; fill in Client Monthly Revenue and Reward Amount (${RATE}%); drag the deal to Commission Approved. ${X}.` },
    ],
  },
  {
    id: 'wf-comm2',
    ...
    summary: FEE ? "Staff approved the partner's fee. GHL records it and asks for the payout." : "Staff approved this month's commission. GHL records it and asks for the payout.",
    layout: { nodeWidth: 470 },
    steps: [
      ...
      { type: 'task', kicker: 'Add task', title: '#1 Payout task', desc: `Pay the partner${FEE ? ` ${RATE}` : ''} through their payment preference, then drag the deal to Commission Paid. ${X}.` },
    ],
  },
  {
    id: 'wf-comm3',
    type: 'flow',
    title: COMM3,
    ghlId: ID('workflows', COMM3),
    summary: FEE ? 'The partner was paid. The fee is closed: nothing repeats.' : 'The partner was paid. A month later the next commission opens by itself, unless the client has left.',
    ...
    steps: [
      ...
      ...(FEE ? [{ type: 'end', title: 'End', desc: 'One fee per deal: nothing repeats.' }] : [{ type: 'wait', title: 'Wait 30 days', desc: 'One month between commissions.', note: `Tested ${TEST_DATE} with the wait at 1 minute. Both the repeat and the stop worked.` },
      {
  ...
  // brings in a client), row 3 left to right (where the lead shows up).
  {
    ...
    nodes: [
      ...
      { id: 'payouts', at: [2, 2], type: 'info', planned: true, kicker: 'Later, optional', title: 'Commissions in the portal', desc: `Needs ${CLIENT}'s Stripe and a sales campaign with a % commission. Until then the ${FEE ? `${RATE} fee` : `${RATE}%`} runs in Referred Leads ({p:pl-referred}), paid by hand.` },
    ],
    edges: [
  ...
  // ================================================================= AT A GLANCE
  {
    ...
    groups: [
      ...
      },
      {
        ...
        note: `Bold tags are added by a workflow. The commission tags are not used: the ${FEE ? 'commission' : 'monthly cycle'} keeps its state on the deal instead.`,
        cards: [
